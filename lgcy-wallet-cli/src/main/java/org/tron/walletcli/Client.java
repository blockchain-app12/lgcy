...
public class Client {
  ...
  private static String[] commandHelp = {
      ...
      "CreateWitness",
      "DeleteProposal",
      "DeployContract contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent value token_value token_id <library:address,library:address,...> <lib_compiler_version(e.g:v5)>",
      "ExchangeCreate",
      "ExchangeInject",
      "ExchangeTransaction",
      "ExchangeWithdraw",
      ...
      // "UpdateAsset",
      "UpdateBrokerage",
      // "UpdateKandyLimit contract_address kandy_limit",
  ...
  private void assetIssue(String[] parameters)
    ...
    if (parameters == null || parameters.length < 12) {
      ...
      System.out.println("AssetIssue [OwnerAddress] AssetName AbbrName TotalSupply USDLNum AssetNum Precision "
              + "StartDate EndDate Description Url"
              + "FrozenAmount0 FrozenDays0 ... FrozenAmountN FrozenDaysN");
      System.out.println("USDLNum and AssetNum represents the conversion ratio of the tron to the asset.");
    ...
    String usdlNumStr = parameters[index++];
    ...
    int usdlNum = new Integer(usdlNumStr);
    ...
    boolean result = walletApiWrapper.assetIssue(ownerAddress, name, abbrName, totalSupply,
        usdlNum, icoNum, precision, startTime, endTime, 0,
        description, url, frozenSupply);
    if (result) {
  ...
  private void listWitnesses() {
    ...
    if (result.isPresent()) {
      ...
      System.out.println("WitnessList" + Utils.formatMessageString(witnessList));
      int count = result.get().getWitnessesCount();
      System.out.println("WitnessList_Count" + count);
    } else {
  ...
  private void freezeBalance(String[] parameters)
    ...
    if (parameters == null || !(parameters.length == 2 || parameters.length == 3
      ...
      System.out.println("freezeBalance [OwnerAddress] frozen_balance frozen_duration "
          + "[ResourceCode:1 USDL_POWER] [receiverAddress]");
      return;
    }
  ...
  private void unfreezeBalance(String[] parameters)
    ...
    if (parameters == null || parameters.length < 1 || parameters.length > 3) {
      ...
      System.out.println(
          "unfreezeBalance [OwnerAddress] ResourceCode(1 USDL_POWER) [receiverAddress]");
      return;
    }
...
//  }
  ...
  // private void updateKandyLimit(String[] parameters)
  //     throws IOException, CipherException, CancelException {
  //   if (parameters == null || (parameters.length != 2 && parameters.length != 3)) {
  //     System.out.println("Using updateKandyLimit command needs 2 parameters like: ");
  //     System.out.println("updateKandyLimit [OwnerAddress] contract_address kandy_limit");
  //     return;
  //   }
  ...
  //   long originKandyLimit = Long.valueOf(parameters[index++]).longValue();
  //   if (originKandyLimit < 0) {
  //     System.out.println("origin_kandy_limit need > 0 ");
  //     return;
  //   }
  //   boolean result = walletApiWrapper
  //       .updateKandyLimit(ownerAddress, contractAddress, originKandyLimit);
  //   if (result) {
  //     System.out.println("UpdateSetting for origin_kandy_limit successful !!!");
  //   } else {
  //     System.out.println("UpdateSetting for origin_kandy_limit failed !!!");
  //   }
  // }
  ...
  private void deployContract(String[] parameter)
    ...
    if (parameters == null ||
      ...
      System.out.println(
          "DeployContract [ownerAddress] contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent value token_value token_id(e.g: USDLTOKEN, use # if don't provided) <library:address,library:address,...> <lib_compiler_version(e.g:v5)>");
//      System.out.println(
//          "Note: Please append the param for constructor tightly with byteCode without any space");
    ...
    long originKandyLimit = Long.parseLong(parameters[idx++]);
    if (consumeUserResourcePercent > 100 || consumeUserResourcePercent < 0) {
      System.out.println("consume_user_resource_percent should be >= 0 and <= 100");
      return;
    }
    // if (originKandyLimit <= 0) {
    //   System.out.println("origin_kandy_limit must > 0");
    //   return;
    // }
    ...
    boolean result = walletApiWrapper
        .deployContract(ownerAddress, contractName, abiStr, codeStr, feeLimit, value,
            consumeUserResourcePercent, originKandyLimit, tokenValue, tokenId, libraryAddressPair,
            compilerVersion);
    if (result) {
  ...
  private void triggerContract(String[] parameters, boolean isConstant)
    ...
    if (isConstant) {
      ...
      if (parameters == null || (parameters.length != 8 && parameters.length != 9)) {
        ...
        System.out.println(cmdMethodStr + " [OwnerAddress] contractAddress method args isHex"
            + " fee_limit value token_value token_id(e.g: USDLTOKEN, use # if don't provided)");
        return;
      }
    }
  ...
  private void listShieldedNote(String[] parameters) {
    ...
    if (showType == 0) {
      ...
      for (Entry<Long, ShieldedNoteInfo> entry : noteMap.entrySet()) {
        ...
        string += " ";
        string += entry.getValue().getUsdlId();
        string += " ";
  ...
  private void run() {
    System.out.println(" ");
    System.out.println("Welcome to LGCY Wallet-Cli");
      ...
      while (true) {
        ...
        try {
          ...
          switch (cmdLowerCase) {
            ...
            // case "updatekandylimit": {
            //   updateKandyLimit(parameters);
            //   break;
            // }
            case "deploycontract": {
