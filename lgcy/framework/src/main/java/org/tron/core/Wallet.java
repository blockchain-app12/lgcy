...
public class Wallet {
  ...
  public Account getAccount(Account account) {
    ...
    KandyProcessor kandyProcessor = new KandyProcessor(
        chainBaseManager.getDynamicPropertiesStore(),
        chainBaseManager.getAccountStore());
    kandyProcessor.updateUsage(accountCapsule);
    ...
    accountCapsule.setLatestConsumeTimeForPower(genesisTimeStamp
        + BLOCK_PRODUCED_INTERVAL * accountCapsule.getLatestConsumeTimeForPower());
  ...
  private void setTransaction(TransactionCapsule usdl) {
    try {
      BlockId blockId = chainBaseManager.getHeadBlockId();
      if ("solid".equals(Args.getInstance().getUsdlReferenceBlock())) {
        blockId = chainBaseManager.getSolidBlockId();
      }
      usdl.setReference(blockId.getNum(), blockId.getBytes());
      long expiration = chainBaseManager.getHeadBlockTimeStamp() + Args.getInstance()
          .getUsdlExpirationTimeInMilliseconds();
  ...
  public TransactionCapsule createTransactionCapsule(com.google.protobuf.Message message,
    ...
    if (contractType != ContractType.CreateSmartContract
      ...
      List<Actuator> actList = ActuatorFactory.createActuator(usdl, chainBaseManager);
    ...
    if (contractType == ContractType.CreateSmartContract) {
      ...
      CreateSmartContract contract = ContractCapsule
          .getSmartContractFromTransaction(usdl.getInstance());
  ...
  public GrpcAPI.Return broadcastTransaction(Transaction signedTransaction) {
    ...
    TransactionCapsule usdl = new TransactionCapsule(signedTransaction);
    usdl.setTime(System.currentTimeMillis());
    try {
      Message message = new TransactionMessage(signedTransaction.toByteArray());
      if (minEffectiveConnection != 0) {
        if (tronNetDelegate.getActivePeer().isEmpty()) {
          logger
              .warn("Broadcast transaction {} has failed, no connection.", usdl.getTransactionId());
        ...
        if (count < minEffectiveConnection) {
          ...
          logger.warn("Broadcast transaction {} has failed, {}.", usdl.getTransactionId(), info);
      ...
      if (dbManager.isTooManyPending()) {
        logger
            .warn("Broadcast transaction {} has failed, too many pending.", usdl.getTransactionId());
        return builder.setResult(false).setCode(response_code.SERVER_BUSY).build();
      }

      if (dbManager.getTransactionIdCache().getIfPresent(usdl.getTransactionId()) != null) {
        logger.warn("Broadcast transaction {} has failed, it already exists.",
            usdl.getTransactionId());
        return builder.setResult(false).setCode(response_code.DUP_TRANSACTION_ERROR).build();
      } else {
        dbManager.getTransactionIdCache().put(usdl.getTransactionId(), true);
      }
      if (chainBaseManager.getDynamicPropertiesStore().supporlvm()) {
        usdl.resetResult();
      }
      dbManager.pushTransaction(usdl);
      tronNetService.broadcast(message);
      logger.info("Broadcast transaction {} successfully.", usdl.getTransactionId());
      return builder.setResult(true).setCode(response_code.SUCCESS).build();
    } catch (ValidateSignatureException e) {
      logger.error(BROADCAST_TRANS_FAILED, usdl.getTransactionId(), e.getMessage());
  ...
  public TransactionApprovedList getTransactionApprovedList(Transaction usdl) {
    ...
    TransactionExtention.Builder usdlExBuilder = TransactionExtention.newBuilder();
    usdlExBuilder.setTransaction(usdl);
    usdlExBuilder.setTxid(ByteString.copyFrom(Sha256Hash.hash(CommonParameter
        .getInstance().isECKeyCryptoEngine(), usdl.getRawData().toByteArray())));
    Return.Builder retBuilder = Return.newBuilder();
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
    usdlExBuilder.setResult(retBuilder);
    tswBuilder.setTransaction(usdlExBuilder);
    TransactionApprovedList.Result.Builder resultBuilder = TransactionApprovedList.Result
        .newBuilder();
    try {
      Contract contract = usdl.getRawData().getContract(0);
      ...
      if (usdl.getSignatureCount() > 0) {
        List<ByteString> approveList = new ArrayList<ByteString>();
        byte[] hash = Sha256Hash.hash(CommonParameter
            .getInstance().isECKeyCryptoEngine(), usdl.getRawData().toByteArray());
        for (ByteString sig : usdl.getSignatureList()) {
          if (sig.size() < 65) {
  ...
  public Protocol.ChainParameters getChainParameters() {
    ...
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getKandyFee")
            .setValue(chainBaseManager.getDynamicPropertiesStore().getKandyFee())
            .build());
    ...
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            ...
            .build());
    //    TOTAL_KANDY_LIMIT, // 50,000,000,000, 17
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getTotalKandyLimit")
            .setValue(chainBaseManager.getDynamicPropertiesStore().getTotalKandyLimit())
            .build());
      //  ALLOW_LVM_TRANSFER_TRC10, // 1, 18
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getAllowLvmTransferTrc10")
            .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowLvmTransferTrc10())
            .build());
      //  TOTAL_CURRENT_KANDY_LIMIT, // 50,000,000,000, 19
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getTotalKandyCurrentLimit")
            .setValue(chainBaseManager.getDynamicPropertiesStore().getTotalKandyCurrentLimit())
            .build());
    ...
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            ...
            .build());
    //    ALLOW_ADAPTIVE_KANDY, // 1, 21
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getAllowAdaptiveKandy")
            .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowAdaptiveKandy())
            .build());
    //other chainParameters
    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getTotalKandyTargetLimit")
        .setValue(chainBaseManager.getDynamicPropertiesStore().getTotalKandyTargetLimit())
        .build());

    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getTotalKandyAverageUsage")
        .setValue(chainBaseManager.getDynamicPropertiesStore().getTotalKandyAverageUsage())
        .build());
    ...
    // ALLOW_LVM_CONSTANTINOPLE
    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getAllowLvmConstantinople")
        .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowLvmConstantinople())
        .build());

    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getAllowLvmSolidity059")
        .setValue(chainBaseManager.getDynamicPropertiesStore().getAllowLvmSolidity059())
        .build());
    
    // ALLOW_LVM_ISTANBUL
    builder.addChainParameter(
        Protocol.ChainParameters.ChainParameter.newBuilder().setKey("getAllowLvmIstanbul")
            .setValue(dbManager.getDynamicPropertiesStore().getAllowLvmIstanbul()).build());
    ...
    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
        .setKey("getAllowLvmStake")
        .setValue(dbManager.getDynamicPropertiesStore().getAllowLvmStake())
        .build());

    builder.addChainParameter(Protocol.ChainParameters.ChainParameter.newBuilder()
            .setKey("getAllowLvmAssetIssue")
            .setValue(dbManager.getDynamicPropertiesStore().getAllowLvmAssetIssue())
            .build());
  ...
  public AccountNetMessage getAccountNet(ByteString accountAddress) {
    ...
    builder.setNetUsed(accountCapsule.getNetUsage())
        //.setFreeNetUsed(accountCapsule.getFreeNetUsage())
  ...
  public AccountResourceMessage getAccountResource(ByteString accountAddress) {
    ...
    long totalNetLimit = chainBaseManager.getDynamicPropertiesStore().getTotalNetLimit();
    long totalNetWeight = chainBaseManager.getDynamicPropertiesStore().getTotalNetWeight();
    long kandyLimit = kandyProcessor
        .calculateGlobalKandyLimit(accountCapsule);
    long totalKandyLimit =
        chainBaseManager.getDynamicPropertiesStore().getTotalKandyCurrentLimit();
    long totalKandyWeight =
        chainBaseManager.getDynamicPropertiesStore().getTotalKandyWeight();
    ...
    builder.setNetUsed(accountCapsule.getNetUsage())
        ...
        .setTotalNetLimit(totalNetLimit)
        .setTotalNetWeight(totalNetWeight)
        .setKandyLimit(kandyLimit)
        .setKandyUsed(accountCapsule.getAccountResource().getKandyUsage())
        .setTotalKandyLimit(totalKandyLimit)
        .setTotalKandyWeight(totalKandyWeight)
  ...
  public Transaction deployContract(TransactionCapsule usdlCap) {

    // do nothing, so can add some useful function later
    // usdlCap contract para cacheUnpackValue has value

    return usdlCap.getInstance();
  }

  public Transaction triggerContract(TriggerSmartContract
      triggerSmartContract,
      TransactionCapsule usdlCap, Builder builder,
    ...
    if (isConstant(abi, selector)) {
      return callConstantContract(usdlCap, builder, retBuilder);
    } else {
      return usdlCap.getInstance();
    }
  }
  ...
  public Transaction callConstantContract(TransactionCapsule usdlCap, Builder
      builder,
      Return.Builder retBuilder)
      throws ContractValidateException, ContractExeException, HeaderNotFound, VMIllegalException {
    ...
    TransactionContext context = new TransactionContext(new BlockCapsule(headBlock), usdlCap,
  ...
  public ShieldedTRC20Parameters createShieldedContractParameters(
    ...
    byte[] shieldedTRC20ContractAddressLvm = new byte[20];
    System.arraycopy(shieldedTRC20ContractAddress, 1, shieldedTRC20ContractAddressLvm, 0, 20);
    builder.setShieldedTRC20Address(shieldedTRC20ContractAddressLvm);
    ...
    if (scaledFromAmount > 0 && spendSize == 0 && receiveSize == 1
      ...
      byte[] transparentToAddressLvm = new byte[20];
      System.arraycopy(transparentToAddress, 1, transparentToAddressLvm, 0, 20);
      builder.setTransparentToAddress(transparentToAddressLvm);
  ...
  private boolean isShieldedTRC20NoteSpent(GrpcAPI.Note note, long pos, byte[] ak,
    ...
    Return.Builder retBuilder = Return.newBuilder();
    TransactionExtention usdlExt;

    try {
      TransactionCapsule usdlCap = createTransactionCapsule(trigger,
          ContractType.TriggerSmartContract);
      Transaction usdl = triggerConstantContract(trigger, usdlCap, usdlExtBuilder, retBuilder);

      retBuilder.setResult(true).setCode(response_code.SUCCESS);
      usdlExtBuilder.setTransaction(usdl);
      usdlExtBuilder.setTxid(usdlCap.getTransactionId().getByteString());
      ...
      usdlExt = usdlExtBuilder.build();
    }

    String code = usdlExt.getResult().getCode().toString();
    if ("SUCCESS".equals(code)) {
      List<ByteString> list = usdlExt.getConstantResultList();
