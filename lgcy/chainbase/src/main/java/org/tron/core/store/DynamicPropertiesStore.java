...
public class DynamicPropertiesStore extends TronStoreWithRevoking<BytesCapsule> {
  ...
  private static final byte[] KANDY_FEE = "KANDY_FEE".getBytes();
  ...
  //This value is only allowed to be 0, 1, -1
  private static final byte[] ALLOW_ADAPTIVE_KANDY = "ALLOW_ADAPTIVE_KANDY".getBytes();
  //This value is only allowed to be 0, 1, -1
  ...
  //This value is only allowed to be 0, 1, -1
  private static final byte[] ALLOW_LVM_TRANSFER_TRC10 = "ALLOW_LVM_TRANSFER_TRC10".getBytes();
  ...
  private static final byte[] ALLOW_SHIELDED_TRC20_TRANSACTION =
      "ALLOW_SHIELDED_TRC20_TRANSACTION"
          .getBytes();
  private static final byte[] ALLOW_LVM_ISTANBUL = "ALLOW_LVM_ISTANBUL".getBytes();
  private static final byte[] ALLOW_LVM_STAKE = "ALLOW_LVM_STAKE".getBytes();
  private static final byte[] ALLOW_LVM_ASSET_ISSUE = "ALLOW_LVM_ASSET_ISSUE".getBytes();
  private static final byte[] ALLOW_LVM_CONSTANTINOPLE = "ALLOW_LVM_CONSTANTINOPLE".getBytes();
  private static final byte[] ALLOW_LVM_SOLIDITY_059 = "ALLOW_LVM_SOLIDITY_059".getBytes();
  ...
  // private static final byte[] BURN_USDL_AMOUNT = "BURN_USDL_AMOUNT".getBytes();
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      ...
      this.saveWitnessPayPerBlock(500000L);
    }
    ...
    try {
      ...
      this.saveAccountUpgradeCost(500_000_000L);
    }
    ...
    try {
      this.getTotalKandyWeight();
    } catch (IllegalArgumentException e) {
      this.saveTotalKandyWeight(0L);
    }

    try {
      this.getAllowAdaptiveKandy();
    } catch (IllegalArgumentException e) {
      this.saveAllowAdaptiveKandy(CommonParameter.getInstance()
          .getAllowAdaptiveKandy());
    }
    ...
    try {
      this.getTotalKandyLimit();
    } catch (IllegalArgumentException e) {
      this.saveTotalKandyLimit(0L);
      //this.saveTotalKandyLimit(50_000_000_000L);
    }

    try {
      this.getKandyFee();
    } catch (IllegalArgumentException e) {
      this.saveKandyFee(1);// 100 sun per kandy
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(100_000L); // 0.1USDL
    }
    ...
    try {
      ...
      this.saveTransactionFee(800L); // 10sun/byte
    }
    ...
    try {
      this.getAllowLvmTransferTrc10();
    } catch (IllegalArgumentException e) {
      this.saveAllowLvmTransferTrc10(CommonParameter.getInstance()
          .getAllowLvmTransferTrc10());
    }

    try {
      this.getAllowLvmConstantinople();
    } catch (IllegalArgumentException e) {
      this.saveAllowLvmConstantinople(CommonParameter.getInstance()
          .getAllowLvmConstantinople());
    }

    try {
      this.getAllowLvmSolidity059();
    } catch (IllegalArgumentException e) {
      this.saveAllowLvmSolidity059(CommonParameter.getInstance()
          .getAllowLvmSolidity059());
    }
    ...
    try {
      this.getAllowLvmIstanbul();
    } catch (IllegalArgumentException e) {
      this.saveAllowLvmIstanbul(
          CommonParameter.getInstance().getAllowLvmIstanbul());
    }

    try {
      this.getAllowLvmStake();
    } catch (IllegalArgumentException e) {
      this.saveAllowLvmStake(
              CommonParameter.getInstance().getAllowLvmStake());
    }

    try {
      this.getAllowLvmAssetIssue();
    } catch (IllegalArgumentException e) {
      this.saveAllowLvmAssetIssue(
              CommonParameter.getInstance().getAllowLvmAssetIssue());
    }
    ...
    try {
      this.getTotalKandyCurrentLimit();
    } catch (IllegalArgumentException e) {
      this.saveTotalKandyCurrentLimit(getTotalKandyLimit());
    }

    try {
      this.getTotalKandyTargetLimit();
    } catch (IllegalArgumentException e) {
      this.saveTotalKandyTargetLimit(getTotalKandyLimit() / 14400);
    }

    try {
      this.getTotalKandyAverageUsage();
    } catch (IllegalArgumentException e) {
      this.saveTotalKandyAverageUsage(0);
    }
    ...
    try {
      this.getTotalKandyAverageTime();
    } catch (IllegalArgumentException e) {
      this.saveTotalKandyAverageTime(0);
    }

    try {
      this.getBlockKandyUsage();
    } catch (IllegalArgumentException e) {
      this.saveBlockKandyUsage(0);
    }
    ...
    //  try {
    //   this.getBurnUsdlAmount();
    // } catch (IllegalArgumentException e) {
    //   this.saveBurnUsdl(0L);
    // }
  ...
  public void saveTotalKandyWeight(long totalKandyWeight) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_WEIGHT,
        new BytesCapsule(ByteArray.fromLong(totalKandyWeight)));
  }

  public long getTotalKandyWeight() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KANDY_WEIGHT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_KANDY_WEIGHT"));
  }
  ...
  @Deprecated
  public void saveTotalKandyLimit(long totalKandyLimit) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_LIMIT,
        new BytesCapsule(ByteArray.fromLong(totalKandyLimit)));

    long ratio = getAdaptiveResourceLimitTargetRatio();
    saveTotalKandyTargetLimit(totalKandyLimit / ratio);
  }

  public void saveTotalKandyLimit2(long totalKandyLimit) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_LIMIT,
        new BytesCapsule(ByteArray.fromLong(totalKandyLimit)));

    long ratio = getAdaptiveResourceLimitTargetRatio();
    saveTotalKandyTargetLimit(totalKandyLimit / ratio);
    if (getAllowAdaptiveKandy() == 0) {
      saveTotalKandyCurrentLimit(totalKandyLimit);
    }
  }

  public long getTotalKandyLimit() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KANDY_LIMIT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_KANDY_LIMIT"));
  }

  public void saveTotalKandyCurrentLimit(long totalKandyCurrentLimit) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_CURRENT_LIMIT,
        new BytesCapsule(ByteArray.fromLong(totalKandyCurrentLimit)));
  }

  public long getTotalKandyCurrentLimit() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KANDY_CURRENT_LIMIT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_KANDY_CURRENT_LIMIT"));
  }

  public void saveTotalKandyTargetLimit(long targetTotalKandyLimit) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_TARGET_LIMIT,
        new BytesCapsule(ByteArray.fromLong(targetTotalKandyLimit)));
  }

  public long getTotalKandyTargetLimit() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KANDY_TARGET_LIMIT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_KANDY_TARGET_LIMIT"));
  }

  public void saveTotalKandyAverageUsage(long totalKandyAverageUsage) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_AVERAGE_USAGE,
        new BytesCapsule(ByteArray.fromLong(totalKandyAverageUsage)));
  }

  public long getTotalKandyAverageUsage() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KANDY_AVERAGE_USAGE))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found TOTAL_KANDY_AVERAGE_USAGE"));
  }
  ...
  public void saveTotalKandyAverageTime(long totalKandyAverageTime) {
    this.put(DynamicResourceProperties.TOTAL_KANDY_AVERAGE_TIME,
        new BytesCapsule(ByteArray.fromLong(totalKandyAverageTime)));
  }

  public long getTotalKandyAverageTime() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.TOTAL_KANDY_AVERAGE_TIME))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
  ...
  public void saveBlockKandyUsage(long blockKandyUsage) {
    this.put(DynamicResourceProperties.BLOCK_KANDY_USAGE,
        new BytesCapsule(ByteArray.fromLong(blockKandyUsage)));
  }

  public long getBlockKandyUsage() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.BLOCK_KANDY_USAGE))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found BLOCK_KANDY_USAGE"));
  }

  public void saveKandyFee(long totalKandyFee) {
    this.put(KANDY_FEE,
        new BytesCapsule(ByteArray.fromLong(totalKandyFee)));
  }

  public long getKandyFee() {
    return Optional.ofNullable(getUnchecked(KANDY_FEE))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found KANDY_FEE"));
  }
  ...
  public void saveTotalStoragePool(long usdl) {
    this.put(TOTAL_STORAGE_POOL,
        new BytesCapsule(ByteArray.fromLong(usdl)));
  }
  ...
  public void saveTotalStorageTax(long usdl) {
  ...
  public void saveAllowAdaptiveKandy(long value) {
    this.put(ALLOW_ADAPTIVE_KANDY,
        new BytesCapsule(ByteArray.fromLong(value)));
  }

  public long getAllowAdaptiveKandy() {
    return Optional.ofNullable(getUnchecked(ALLOW_ADAPTIVE_KANDY))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found ALLOW_ADAPTIVE_KANDY"));
  }

  public void saveAllowLvmTransferTrc10(long value) {
    this.put(ALLOW_LVM_TRANSFER_TRC10,
        new BytesCapsule(ByteArray.fromLong(value)));
  }

  public long getAllowLvmTransferTrc10() {
    return Optional.ofNullable(getUnchecked(ALLOW_LVM_TRANSFER_TRC10))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found ALLOW_LVM_TRANSFER_TRC10"));
  }

  public void saveAllowLvmConstantinople(long value) {
    this.put(ALLOW_LVM_CONSTANTINOPLE,
        new BytesCapsule(ByteArray.fromLong(value)));
  }

  public long getAllowLvmConstantinople() {
    return Optional.ofNullable(getUnchecked(ALLOW_LVM_CONSTANTINOPLE))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found ALLOW_LVM_CONSTANTINOPLE"));
  }

  public void saveAllowLvmSolidity059(long value) {
    this.put(ALLOW_LVM_SOLIDITY_059,
        new BytesCapsule(ByteArray.fromLong(value)));
  }

  public long getAllowLvmSolidity059() {
    return Optional.ofNullable(getUnchecked(ALLOW_LVM_SOLIDITY_059))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(() -> new IllegalArgumentException("not found ALLOW_LVM_SOLIDITY_059"));
  }
  ...
  public void updateDynamicStoreByConfig() {
    if (CommonParameter.getInstance()
        .getAllowLvmConstantinople() != 0) {
      saveAllowLvmConstantinople(CommonParameter.getInstance()
          .getAllowLvmConstantinople());
  ...
  public void saveAllowLvmIstanbul(long allowLVMIstanbul) {
    this.put(DynamicPropertiesStore.ALLOW_LVM_ISTANBUL,
        new BytesCapsule(ByteArray.fromLong(allowLVMIstanbul)));
  }

  public long getAllowLvmIstanbul() {
    String msg = "not found ALLOW_LVM_ISTANBUL";
    return Optional.ofNullable(getUnchecked(ALLOW_LVM_ISTANBUL))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException(msg));
  ...
  public void saveAllowLvmStake(long allowLvmStake) {
    this.put(DynamicPropertiesStore.ALLOW_LVM_STAKE,
            new BytesCapsule(ByteArray.fromLong(allowLvmStake)));
  }

  public void saveAllowLvmAssetIssue(long allowLvmAssetIssue) {
    this.put(DynamicPropertiesStore.ALLOW_LVM_ASSET_ISSUE,
            new BytesCapsule(ByteArray.fromLong(allowLvmAssetIssue)));
  }

  public long getAllowLvmStake() {
    String msg = "not found ALLOW_LVM_STAKE";
    return Optional.ofNullable(getUnchecked(ALLOW_LVM_STAKE))
            .map(BytesCapsule::getData)
            .map(ByteArray::toLong)
            .orElseThrow(
                    () -> new IllegalArgumentException(msg));
  ...
  public long getAllowLvmAssetIssue() {
    String msg = "not found ALLOW_LVM_ASSETISSUE";
    return Optional.ofNullable(getUnchecked(ALLOW_LVM_ASSET_ISSUE))
            .map(BytesCapsule::getData)
            .map(ByteArray::toLong)
            .orElseThrow(
                    () -> new IllegalArgumentException(msg));
  ...
  //The unit is usdl
  public void addTotalKandyWeight(long amount) {
    long totalKandyWeight = getTotalKandyWeight();
    totalKandyWeight += amount;
    saveTotalKandyWeight(totalKandyWeight);
  }
  ...
  // public long getBurnUsdlAmount() {
  //   return Optional.ofNullable(getUnchecked(BURN_USDL_AMOUNT))
  //       .map(BytesCapsule::getData)
  //       .map(ByteArray::toLong)
  //       .orElseThrow(() -> new IllegalArgumentException("not found BURN_USDL_AMOUNT"));
  // }

  // public void burnUsdl(long amount) {
  //   if (amount <= 0) {
  //     return;
  //   }
  //   amount += getBurnUsdlAmount();
  //   saveBurnUsdl(amount);
  // }

  // private void saveBurnUsdl(long amount) {
  //   this.put(BURN_USDL_AMOUNT, new BytesCapsule(ByteArray.fromLong(amount)));
  // }
  ...
  private static class DynamicResourceProperties {
    ...
    private static final byte[] TOTAL_KANDY_TARGET_LIMIT = "TOTAL_KANDY_TARGET_LIMIT".getBytes();
    private static final byte[] TOTAL_KANDY_CURRENT_LIMIT = "TOTAL_KANDY_CURRENT_LIMIT"
        .getBytes();
    private static final byte[] TOTAL_KANDY_AVERAGE_USAGE = "TOTAL_KANDY_AVERAGE_USAGE"
        .getBytes();
    private static final byte[] TOTAL_KANDY_AVERAGE_TIME = "TOTAL_KANDY_AVERAGE_TIME".getBytes();
    private static final byte[] TOTAL_KANDY_WEIGHT = "TOTAL_KANDY_WEIGHT".getBytes();
    private static final byte[] TOTAL_KANDY_LIMIT = "TOTAL_KANDY_LIMIT".getBytes();
    private static final byte[] BLOCK_KANDY_USAGE = "BLOCK_KANDY_USAGE".getBytes();
